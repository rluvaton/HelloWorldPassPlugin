...
static cl::opt<bool> UsePostIncrementRanges(
    "indvars-post-increment-ranges-2", cl::Hidden,
    cl::desc("Use post increment control-dependent ranges in IndVarSimplify"),
    cl::init(true));

static cl::opt<bool>
    DisableLFTR("disable-lftr-2", cl::Hidden, cl::init(false),
                cl::desc("Disable Linear Function Test Replace optimization"));

static cl::opt<bool>
    LoopPredication("indvars-predicate-loops-2", cl::Hidden, cl::init(true),
                    cl::desc("Predicate conditions in read only loops"));

static cl::opt<bool>
    AllowIVWidening("indvars-widen-indvars-2", cl::Hidden, cl::init(true),
...
bool IndVarSimplify::linearFunctionTestReplace(Loop *L, BasicBlock *ExitingBB,
  ...
  llvm::errs() << "INDVARS: Rewriting loop exit condition to:\n"
               << "      LHS:" << *CmpIndVar << '\n'
...
static bool optimizeLoopExitWithUnknownExitCount(
  ...
  for (auto *OldCond : LeafConditions) {
    ...
    if (auto Replaced =
      ...
      llvm::errs() << "Unknown exit count: Replacing " << *OldCond << " with "
                   << *NewCond << "\n";
...
bool IndVarSimplify::run(Loop *L) {
  ...
  if (!L->isLoopSimplifyForm()) {
    llvm::errs() << "L->isLoopSimplifyForm(): false\n";
  ...
  if (ReplaceExitValue != NeverRepl) {
    ...
    if (int Rewrites = rewriteLoopExitValues(L, LI, TLI, SE, TTI, Rewriter, DT,
      ...
      llvm::errs() << "rewriteLoopExitValues false\n";
...
//   // If we decided that there are basic blocks that are dead in this
//   function,
...
//   L.getInductionVariable(ScalarEvolutionAnalysisResult); if
//   (InductionVariable) {
...
//     const SCEV *InductionSCEV =
//     ScalarEvolutionAnalysisResult.getSCEV(InductionVariable); errs() <<
//     "Induction variable SCEV: "; InductionSCEV->print(errs()); errs() <<
//     "\n";
//   } else {
