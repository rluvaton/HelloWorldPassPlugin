...
void initializeLoopPassPass(PassRegistry &Registry) {
...
/// different.
void addStringMetadataToLoop(Loop *TheLoop, const char *StringMD, unsigned V) {
...
std::optional<MDNode *>
makeFollowupLoopID(MDNode *OrigLoopID, ArrayRef<StringRef> FollowupOptions,
...
llvm::MyLoopUtils::TransformationMode hasUnrollTransformation(const Loop *L) {
  ...
  if (Count)
    return *Count == 1 ? llvm::MyLoopUtils::TM_SuppressedByUser
                       : llvm::MyLoopUtils::TM_ForcedByUser;
...
llvm::MyLoopUtils::TransformationMode
hasUnrollAndJamTransformation(const Loop *L) {
...
llvm::MyLoopUtils::TransformationMode
hasVectorizeTransformation(const Loop *L) {
...
llvm::MyLoopUtils::TransformationMode
hasDistributeTransformation(const Loop *L) {
...
llvm::MyLoopUtils::TransformationMode
hasLICMVersioningTransformation(const Loop *L) {
...
bool isAlmostDeadIV(PHINode *PN, BasicBlock *LatchBlock, Value *Cond) {
...
void breakLoopBackedge(llvm::Loop *L, llvm::DominatorTree &DT,
  ...
  llvm::Loop *OutermostLoop = L->getOutermostLoop();
  ...
  if (OutermostLoop != L)
    llvm::MyLoopUtils::formLCSSARecursively(*OutermostLoop, DT, &LI, &SE);
}
...
Value *createFindLastIVReduction(IRBuilderBase &Builder, Value *Src,
...
Value *getReductionIdentity(Intrinsic::ID RdxID, Type *Ty,
                            FastMathFlags Flags) {
  bool Negative = false;
  switch (RdxID) {
  default:
...
Value *createSimpleReduction(IRBuilderBase &Builder, Value *Src, RecurKind Kind,
...
int rewriteLoopExitValues(Loop *L, LoopInfo *LI, TargetLibraryInfo *TLI,
  ...
  for (BasicBlock *ExitBB : ExitBlocks) {
    ...
    while ((PN = dyn_cast<PHINode>(BBI++))) {
      ...
      for (unsigned i = 0; i != NumPreds; ++i) {
        ...
        // to do so.
        if (ReplaceExitValue == llvm::MyLoopUtils::UnusedIndVarInLoop) {
          InductionDescriptor ID;
  ...
  for (const RewritePhi &Phi : RewritePhiSet) {
    ...
    if ((ReplaceExitValue == llvm::MyLoopUtils::OnlyCheapRepl ||
         ReplaceExitValue == llvm::MyLoopUtils::UnusedIndVarInLoop) &&
  ...
  auto HasNoClobbersOnPath =
      [&L, &AA, &AccessedLocs, &ExitingBlocks, &InstToDuplicate,
       MSSAThreshold](BasicBlock *Succ, BasicBlock *Header,
                      SmallVector<MemoryAccess *, 4> AccessesToCheck)
      -> std::optional<llvm::MyLoopUtils::IVConditionInfo> {
    llvm::MyLoopUtils::IVConditionInfo Info;
