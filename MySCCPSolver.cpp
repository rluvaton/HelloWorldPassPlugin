//===- MySCCPSolver.cpp - SCCP Utility --------------------------- *- C++ -*-===//
//
...
bool MySCCPSolver::isConstant(const ValueLatticeElement &LV) {
  return LV.isConstant() ||
         (LV.isConstantRange() && LV.getConstantRange().isSingleElement());
}

bool MySCCPSolver::isOverdefined(const ValueLatticeElement &LV) {
  return !LV.isUnknownOrUndef() && !MySCCPSolver::isConstant(LV);
}

bool MySCCPSolver::tryToReplaceWithConstant(Value *V) {
  ...
  if (CB && ((CB->isMustTailCall() && !wouldInstructionBeTriviallyDead(CB)) ||
    ...
    llvm::errs() << "  Can\'t treat the result of call " << *CB
                      << " as a constant\n";
    return false;
  }

  llvm::errs() << "  Constant: " << *Const << " = " << *V << '\n';
...
/// them.
static ConstantRange getRange(Value *Op, MySCCPSolver &Solver,
...
static bool refineInstruction(MySCCPSolver &Solver,
                              const SmallPtrSetImpl<Value *> &InsertedValues,
                              Instruction &Inst) {
  bool Changed = false;
...
static bool replaceSignedInst(MySCCPSolver &Solver,
...
static Value *simplifyInstruction(MySCCPSolver &Solver,
                                  SmallPtrSetImpl<Value *> &InsertedValues,
                                  Instruction &Inst) {
  auto GetRange = [&Solver, &InsertedValues](Value *Op) {
    return getRange(Op, Solver, InsertedValues);
...
bool MySCCPSolver::simplifyInstsInBlock(BasicBlock &BB,
                                      SmallPtrSetImpl<Value *> &InsertedValues,
...
bool MySCCPSolver::removeNonFeasibleEdges(BasicBlock *BB, DomTreeUpdater &DTU,
...
/// Helper class for MySCCPSolver. This implements the instruction visitor and
/// holds all the state.
...
private:
  ...
  void invalidate(CallBase *Call) {
    ...
    while (!ToInvalidate.empty()) {
      ...
      if (V) {
        llvm::errs() << "Invalidated lattice for " << *V << "\n";
...
bool SCCPInstVisitor::markBlockExecutable(BasicBlock *BB) {
  if (!BBExecutable.insert(BB).second)
    return false;
  llvm::errs() << "Marking Block Executable: " << BB->getName() << '\n';
...
bool SCCPInstVisitor::markConstantRange(ValueLatticeElement &IV, Value *V,
  ...
  if (!IV.markConstantRange(CR))
    return false;
  (llvm::errs() << "markConstantRange: " << CR << ": " << *V << '\n');
  pushUsersToWorkList(V);
  return true;
}
...
bool SCCPInstVisitor::markOverdefined(ValueLatticeElement &IV, Value *V) {
  ...
  llvm::errs() << "markOverdefined: ";
             if (auto *F = dyn_cast<Function>(V)) llvm::errs()
             << "Function '" << F->getName() << "'\n";
             else llvm::errs() << *V << '\n';
...
Constant *SCCPInstVisitor::getConstantOrNull(Value *V) const {
  ...
  if (V->getType()->isStructTy()) {
    ...
    if (any_of(LVs, MySCCPSolver::isOverdefined))
      return nullptr;
    std::vector<Constant *> ConstVals;
    auto *ST = cast<StructType>(V->getType());
    for (unsigned I = 0, E = ST->getNumElements(); I != E; ++I) {
      ValueLatticeElement LV = LVs[I];
      ConstVals.push_back(MySCCPSolver::isConstant(LV)
    ...
    if (MySCCPSolver::isOverdefined(LV))
      return nullptr;
    Const = MySCCPSolver::isConstant(LV) ? getConstant(LV, V->getType())
...
void SCCPInstVisitor::setLatticeValueForSpecializationArguments(Function *F,
  ...
  for (auto End = F->arg_end(); NewArg != End; ++NewArg, ++OldArg) {
    ...
    llvm::errs() << "SCCP: Marking argument "
...
void SCCPInstVisitor::visitInstruction(Instruction &I) {
  ...
  // go to overdefined.
  llvm::errs() << "SCCP: Don't know how to handle: " << I << '\n';
  markOverdefined(&I);
}
...
bool SCCPInstVisitor::mergeInValue(ValueLatticeElement &IV, Value *V,
  ...
  if (IV.mergeIn(MergeWithV, Opts)) {
    pushUsersToWorkList(V);
    llvm::errs() << "Merged " << MergeWithV << " into " << *V << " : "
                      << IV << "\n";
    return true;
  }
  return false;
...
bool SCCPInstVisitor::markEdgeExecutable(BasicBlock *Source, BasicBlock *Dest) {
  ...
  if (!markBlockExecutable(Dest)) {
    ...
    llvm::errs() << "Marking Edge Executable: " << Source->getName()
                      << " -> " << Dest->getName() << '\n';
...
void SCCPInstVisitor::getFeasibleSuccessors(Instruction &TI,
  ...
  llvm::errs() << "Unknown terminator instruction: " << TI << '\n';
...
void SCCPInstVisitor::visitUnaryOperator(Instruction &I) {
  ...
  if (MySCCPSolver::isConstant(V0State))
...
void SCCPInstVisitor::visitBinaryOperator(Instruction &I) {
  ...
  if ((V1State.isConstant() || V2State.isConstant())) {
    Value *V1 = MySCCPSolver::isConstant(V1State)
                    ? getConstant(V1State, I.getOperand(0)->getType())
                    : I.getOperand(0);
    Value *V2 = MySCCPSolver::isConstant(V2State)
...
void SCCPInstVisitor::visitCmpInst(CmpInst &I) {
  ...
  if ((V1State.isUnknownOrUndef() || V2State.isUnknownOrUndef()) &&
      !MySCCPSolver::isConstant(ValueState[&I]))
    return;
...
void SCCPInstVisitor::visitLoadInst(LoadInst &I) {
  ...
  if (MySCCPSolver::isConstant(PtrVal)) {
...
void SCCPInstVisitor::handleCallOverdefined(CallBase &CB) {
  ...
  if (F && F->isDeclaration() && canConstantFoldCallTo(&CB, F)) {
    ...
    for (const Use &A : CB.args()) {
      ...
      if (State.isUnknownOrUndef())
        return; // Operands are not resolved yet.
      if (MySCCPSolver::isOverdefined(State))
        return (void)markOverdefined(&CB);
      assert(MySCCPSolver::isConstant(State) && "Unknown state!");
      Operands.push_back(getConstant(State, A->getType()));
    }

    if (MySCCPSolver::isOverdefined(getValueState(&CB)))
      return (void)markOverdefined(&CB);
...
//===----------------------------------------------------------------------===//
//
// MySCCPSolver implementations
//
MySCCPSolver::MySCCPSolver(
    const DataLayout &DL,
    std::function<const TargetLibraryInfo &(Function &)> GetTLI,
    LLVMContext &Ctx)
    : Visitor(new SCCPInstVisitor(DL, std::move(GetTLI), Ctx)) {}

MySCCPSolver::~MySCCPSolver() = default;

void MySCCPSolver::addPredicateInfo(Function &F, DominatorTree &DT,
                                  AssumptionCache &AC) {
  Visitor->addPredicateInfo(F, DT, AC);
}

void MySCCPSolver::removeSSACopies(Function &F) {
  Visitor->removeSSACopies(F);
}

bool MySCCPSolver::markBlockExecutable(BasicBlock *BB) {
  return Visitor->markBlockExecutable(BB);
}

const PredicateBase *MySCCPSolver::getPredicateInfoFor(Instruction *I) {
  return Visitor->getPredicateInfoFor(I);
}

void MySCCPSolver::trackValueOfGlobalVariable(GlobalVariable *GV) {
  Visitor->trackValueOfGlobalVariable(GV);
}

void MySCCPSolver::addTrackedFunction(Function *F) {
  Visitor->addTrackedFunction(F);
}

void MySCCPSolver::addToMustPreserveReturnsInFunctions(Function *F) {
  Visitor->addToMustPreserveReturnsInFunctions(F);
}

bool MySCCPSolver::mustPreserveReturn(Function *F) {
  return Visitor->mustPreserveReturn(F);
}

void MySCCPSolver::addArgumentTrackedFunction(Function *F) {
  Visitor->addArgumentTrackedFunction(F);
}

bool MySCCPSolver::isArgumentTrackedFunction(Function *F) {
  return Visitor->isArgumentTrackedFunction(F);
}

const SmallPtrSetImpl<Function *> &
MySCCPSolver::getArgumentTrackedFunctions() const {
  return Visitor->getArgumentTrackedFunctions();
}

void MySCCPSolver::solve() { Visitor->solve(); }

bool MySCCPSolver::resolvedUndefsIn(Function &F) {
  return Visitor->resolvedUndefsIn(F);
}

void MySCCPSolver::solveWhileResolvedUndefsIn(Module &M) {
  Visitor->solveWhileResolvedUndefsIn(M);
}

void
MySCCPSolver::solveWhileResolvedUndefsIn(SmallVectorImpl<Function *> &WorkList) {
  Visitor->solveWhileResolvedUndefsIn(WorkList);
}

void MySCCPSolver::solveWhileResolvedUndefs() {
  Visitor->solveWhileResolvedUndefs();
}

bool MySCCPSolver::isBlockExecutable(BasicBlock *BB) const {
  return Visitor->isBlockExecutable(BB);
}

bool MySCCPSolver::isEdgeFeasible(BasicBlock *From, BasicBlock *To) const {
  return Visitor->isEdgeFeasible(From, To);
}

std::vector<ValueLatticeElement>
MySCCPSolver::getStructLatticeValueFor(Value *V) const {
  return Visitor->getStructLatticeValueFor(V);
}

void MySCCPSolver::removeLatticeValueFor(Value *V) {
  return Visitor->removeLatticeValueFor(V);
}

void MySCCPSolver::resetLatticeValueFor(CallBase *Call) {
  Visitor->resetLatticeValueFor(Call);
}

const ValueLatticeElement &MySCCPSolver::getLatticeValueFor(Value *V) const {
  return Visitor->getLatticeValueFor(V);
}

const MapVector<Function *, ValueLatticeElement> &
MySCCPSolver::getTrackedRetVals() const {
  return Visitor->getTrackedRetVals();
}

const DenseMap<GlobalVariable *, ValueLatticeElement> &
MySCCPSolver::getTrackedGlobals() const {
  return Visitor->getTrackedGlobals();
}

const SmallPtrSet<Function *, 16> &MySCCPSolver::getMRVFunctionsTracked() const {
  return Visitor->getMRVFunctionsTracked();
}

void MySCCPSolver::markOverdefined(Value *V) { Visitor->markOverdefined(V); }

void MySCCPSolver::trackValueOfArgument(Argument *V) {
  Visitor->trackValueOfArgument(V);
}

bool MySCCPSolver::isStructLatticeConstant(Function *F, StructType *STy) {
  return Visitor->isStructLatticeConstant(F, STy);
}

Constant *MySCCPSolver::getConstant(const ValueLatticeElement &LV,
                                  Type *Ty) const {
  return Visitor->getConstant(LV, Ty);
}

Constant *MySCCPSolver::getConstantOrNull(Value *V) const {
  return Visitor->getConstantOrNull(V);
}

void MySCCPSolver::setLatticeValueForSpecializationArguments(Function *F,
                                   const SmallVectorImpl<ArgInfo> &Args) {
  Visitor->setLatticeValueForSpecializationArguments(F, Args);
}

void MySCCPSolver::markFunctionUnreachable(Function *F) {
  Visitor->markFunctionUnreachable(F);
}

void MySCCPSolver::visit(Instruction *I) { Visitor->visit(I); }

void MySCCPSolver::visitCall(CallInst &I) { Visitor->visitCall(I); }
